  ...
  "OrdersBy" should {
    ...
    "default ordering" should {
      "return query for an element in Context" in {
        OrdersBy(false, personA).toQuery(context) shouldBe "ORDER BY a0"
      }

      "return empty statement if no elements passed" in {
        OrdersBy().toQuery(context) shouldBe ""
      }
      ...
      "return elements for multiple properties" in {
        OrdersBy(false, personA('name, 'age), departmentA('name))
          .toQuery(context) shouldBe "ORDER BY a0.name,a0.age,a1.name"
      }
    ...
    "DESC ordering" should {
      "return query for an element in Context" in {
        OrdersBy(true, personA).toQuery(context) shouldBe "ORDER BY a0 DESC"
      }
